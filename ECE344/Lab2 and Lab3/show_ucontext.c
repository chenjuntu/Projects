...
main(int argc, char **argv)
{
	 ...
	 * getcontext().
         * Answer: mccontext  and sigmask have been changed.
	...
	if (setcontext_called == 1) {
		/* will be get here? why or why not? */
                //Yes, because the setcontext makes the process back to getcontext.
		show_interrupt();
		exit(0);
	}
	...
	/* show size of ucontext_t structure. Hint: use sizeof(). Changed*/
	...
	/* QUESTIONS: Is setcontext_called and err stored on the stack? 
         * setcontext is not called yet. 
         * does the
	...
	printf("number of bytes pushed to the stack between setcontext_called "
	       "and err = %ld\n", (unsigned long)sizeof(setcontext_called));
	...
	printf("number of bytes between err and the saved stack in mycontext "
        ...
        //printf("%p  %p  0x%lx\n", &err, &setcontext_called, (unsigned long)mycontext.uc_mcontext.gregs[10]);
	...
	/* QUESTION: why does the program not fail at the assert below? */
        //Because call_setcontext function reset the process back to the getcontext.
	assert(0);
}
